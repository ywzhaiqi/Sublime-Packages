...
class MarkdownToBbcodeCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        str = self.view.substr(regions)
        contents = markdown_to_bbcode(str)
        self.view.replace(edit, regions, contents)
...
def markdown_to_bbcode(s):
    ...
    s = re.sub(r"(?m)^(\S.*)\n=+\s*$", translate("~[size=6][color=blue][font=Sans Serif]%s[/font][/color][/size]\n"), s)
    s = re.sub(r"(?m)^(\S.*)\n-+\s*$", translate("~[size=5][color=blue][font=Sans Serif]%s[/font][/color][/size]\n"), s)
    s = re.sub(r"(?m)^#\s+(.*?)\s*#*$", translate("~[size=6][color=blue][font=Sans Serif]%s[/font][/color][/size]\n"), s)
    s = re.sub(r"(?m)^##\s+(.*?)\s*#*$", translate("~[size=5][color=blue][font=Sans Serif]%s[/font][/color][/size]\n"), s)
    s = re.sub(r"(?m)^###\s+(.*?)\s*#*$", translate("~[size=4][color=blue][font=Sans Serif]%s[/font][/color][/size]\n"), s)
